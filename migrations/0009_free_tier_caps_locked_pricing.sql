...
--
-- Background: the locked pricing model (canonical doc 2026-06-16, confirmed
-- 2026-07-03) defines free-tier agent caps as HARD caps with no per-agent
-- fees and no unlimited fallthrough:
--
...
--   * domain tier: free_slots_total → 100 and max_agents → 100, but NEVER
--     lower an existing value that is already >= the target (e.g. a
--     registrar-pushed max_agents=1000 stays 1000). max_agents NULL → 100 is
--     an intentional LOWERING from unlimited: the hard-cap model has no
--     unlimited free tier.
--   * email tier: free_slots_total → 10, max_agents 5 → 10, same raise-only
...
-- Domain (Team) tier: free slots 3 → 100 (raise-only).
UPDATE agent_slots
   SET free_slots_total = 100
 WHERE free_slots_total < 100
   AND operator_id IN (
     SELECT id FROM operators WHERE verification_tier = 'domain'
   );

-- Domain (Team) tier: max_agents NULL → 100 (kill the unlimited fallthrough)
-- and raise anything below 100; never lower a cap already >= 100.
UPDATE agent_slots
   SET max_agents = 100
 WHERE (max_agents IS NULL OR max_agents < 100)
   AND operator_id IN (
     SELECT id FROM operators WHERE verification_tier = 'domain'
   );

-- Email (Single Operator) tier: free slots 0 → 10 (raise-only).
UPDATE agent_slots
   SET free_slots_total = 10
 WHERE free_slots_total < 10
   AND operator_id IN (
     SELECT id FROM operators WHERE verification_tier = 'email'
   );

-- Email (Single Operator) tier: max_agents 5 → 10 (raise-only; NULL left
