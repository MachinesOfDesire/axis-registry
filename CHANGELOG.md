...
- **`POST /operators/verify-domain` now returns the persisted `operator.id` for an already-existing operator, not a freshly-minted phantom id.** `handleVerifyDomain` derives a fresh opaque `operatorId` unconditionally, then branches: a NEW operator is INSERTed under that id, but an EXISTING operator (matched by email, the email-tier signup path) is only UPDATEd — its row keeps its original id. The response, however, returned the freshly-minted `operatorId` in both branches, so any **repeat** verify-domain for an email that already had an operator handed the registrar an `operator_id` that was never written. `GET /operators/:id` on it returned 404, and the registrar (`kipple-registrar` / AXIS Prime signup) stored a non-resolving id in `accounts`. Confirmed in production 2026-06-29: `josh.ashcroft@gmail.com` (operator `op-8f3998c35292`, created 2026-04-19) was UPDATEd at 01:11:48 that day while the registrar recorded `op-b4389c15fc3d639c2931fd85`, which 404s. First-time signups were never affected (INSERT then return the same id); only the reconcile path was wrong, and it surfaced as "any email-tier signup" because every web-signup test reused an email that already had an operator. Fix: report `operator.id` when the operator already exists. New `test/integration/verify-domain-reconcile.test.js` covers first signup (resolves), repeat signup (same persisted id, resolves, no duplicate row), and repeat against a pre-seeded operator; the two repeat-path assertions fail without the fix.
- **`POST /operators/verify-domain` no longer clobbers an existing operator's domain or in-flight verification token on an email-only call.** The existing-operator branch ran its `UPDATE operators SET domain = ?, domain_verification_token = ?, …` unconditionally, binding `domain || null`. Because the registrar's signup flow always calls with email only (no domain), every repeat sign-in would NULL out a domain the operator had previously claimed/verified and churn any in-flight verification token. The UPDATE is now guarded by `if (domain)` — it only runs when a domain is actually being claimed (preserving the pre-created-email-operator → domain-claim flow that `handleCheckDomain` depends on); an email-only call against an existing operator is a no-op. Relatedly, the response's `tier` / `domain` / `active` fields now reflect the **persisted operator** rather than the request inputs (`active` = email-tier always active, domain-tier active once verified), so an email-only re-signup of a domain-verified operator reports its real state instead of `tier:'email'` / `domain:null`. Covered by `test/integration/verify-domain-reconcile.test.js` (no-clobber, no-token-churn, and a domain-claim regression that proves the guarded UPDATE still persists a claim).
