...
### Added (v0.x — Governor v0.1 companion)

- **`parent_operator_id` column on `operators` table.** Nullable `TEXT` self-reference; enables a three-level principal hierarchy (organization → user → agent) without breaking the existing two-level model. Existing rows retain `NULL` parent and behave identically. Migration `0006_parent_operator_id.sql` adds the column + `idx_operators_parent` index to live databases; `schema.sql` carries the column inline with a `FOREIGN KEY (parent_operator_id) REFERENCES operators(id)` constraint for fresh deploys. (D1/SQLite cannot add a FK via `ALTER`, so live-DB FK semantics are enforced at the application layer; fresh deploys carry it structurally.) Spec: `docs/specs/axis-registry-v0.x-changes-spec.md`.
- **`POST /operators/verify-domain` accepts optional `parent_operator_id`.** When set, the proposed parent must exist AND belong to the calling registrar; otherwise `400 invalid_parent_operator` or `403 parent_operator_cross_registrar` (the latter is the stable BOLA code distinct from the generic `not_your_resource`). When omitted or `null`, behavior is unchanged: a top-level operator is created.
- **`GET /operators/:id` returns `parent_operator_id` when set.** Exposed in the presentation layer alongside `verification_tier` and `registered_at`; gated behind the same unlock conditions (AIT, owner, admin+). Field is omitted when `NULL` so payloads stay compact.
- **`PATCH /operators/:id` — new endpoint.** Sets or clears `parent_operator_id` on an existing operator. v0.x scope: only `parent_operator_id` is mutable; unknown body fields return `400 invalid_request`. Self-loops (`parent_operator_id` = id) rejected; multi-hop cycle detection deferred to v1.x (open question in the spec). Caller must own the operator (BOLA → `403 not_your_resource`). If setting parent: same-registrar BOLA on the proposed parent (`403 parent_operator_cross_registrar`). Writes an `update_operator_parent` audit row via the standard `logAudit` deferred-write pipeline, capturing both `previous_parent_operator_id` and `new_parent_operator_id`.
- **`GET /operators/:id/children` — new endpoint.** Paginated child operators (matching the `GET /operators` pagination convention). Caller must own the parent or be `admin`+. Plain registrars see only children that also belong to them; `admin`+ sees all children of the parent across registrars (consistent with the existing `GET /operators/:id` admin-read pattern).
- **Integration tests for the v0.x surface (`test/integration/parent-operator-id.test.js`, 17 tests).** Covers the happy paths, every error code, BOLA matrix, audit write, presentation gating, and admin cross-tenant read. The existing delegation-chain coverage is unchanged: `parent_operator_id` is a structural hint at the principal hierarchy and does not alter chain-walk semantics, so the existing operator-to-operator delegation pipeline already supports parent-to-child chains.
- **`docs/scopes.md` — non-normative scope-syntax recommendation.** Implements Change 2 of the v0.x spec. Suggests `service:<server>:<verb>` (coarse) and `service:<server>:<action>` (fine) as shared conventions for independent gateway implementations. Explicitly notes that the registry stores any scope string and does not validate format.
